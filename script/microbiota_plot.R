...
setwd("/Users/yujijun/Documents/01-Work/06-BD_project/BD_projects/centrifuge")
file.list <- list.files(path = ".",pattern = "_addSample_report.txt")
empty = data.frame()
for (i in file.list){
  file_i <- read.delim(file = i,fill = T)
  empty = rbind(empty,file_i)
  print(dim(empty))
}
abundance.input  <- empty
...
#meta <- fread("./metasheet.csv", sep = ",", header = TRUE)
####read in meta data
...
for (clinic_col in clinic.col){
  ...
  if(length(phenos) != 2){
    pheatmap(t(abun.ss), color = my_palette, breaks = bk,border_color = NA, #show_rownames = FALSE,
             ...
             fontsize_row = 12, fontsize_col = 8,show_colnames = T,show_rownames = T)
  }
  ...
  for(clust_phe in phenos){
    ...
    pheatmap(t(ss.abun), color = my_palette, breaks = bk,border_color = NA, show_colnames = T, 
             fontsize_row = 20, fontsize = 25, main = paste("Microbiota Abundace in ","BD", sep = "") )
    dev.off()
}
}
